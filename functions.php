...
// register forntend Ajax
add_action('wp_ajax_date_add_front_end', 'date_add_front_end', 0);
add_action('wp_ajax_nopriv_date_add_front_end', 'date_add_front_end');
function date_add_front_end() {
  $products_id = $_POST["products_id"];
  $date = get_field( "date_products", $products_id );
  $output = '<option value="'.$date.'" selected>' . $date . '</option>';
  echo $output;
	die;
}
...
function financial_add_front_end() {
  ...
  $sub_term   = get_term_by( 'id', $subcategory, 'category' );
    ...
    $output .= '<div class="title"><div class="term"><span>'.$term->name.'</span>, <span>'.$sub_term->name.'</span></div><span>'.$title.'</span></div>';    
    $output .= '<div class="price">amount: '.$amount.' EGP </div>';
    $output .= '<div class="subtotal" price="'.$amount.'"></div>';
    $output .= '</div>';
